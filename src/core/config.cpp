...
ConfigPtr DefaultConfig() { return std::make_shared<Config>(); }
  ...
  if (!doc["HTTP"].isNull()) {
    ...
    if (!http_node["error_logfile"].isNull()) {
      result->error_logfile =
          static_cast<std::filesystem::path>(doc["error_logfile"].asString());
    }
    ...
    if (!http_node["threads"].isNull()) {
      result->threads = std::max<unsigned int>(result->threads,
                                               http_node["threads"].asUInt());
    }

    if (!http_node["connection_timeout"].isNull()) {
      result->connection_timeout = std::min<unsigned int>(
          result->connection_timeout, http_node["connection_timeout"].asUInt());
    }

    if (!http_node["request_timeout"].isNull()) {
      result->request_timeout = std::min<unsigned int>(
          result->request_timeout, http_node["request_timeout"].asUInt());
    }
    return result;
  }
...
GetConfigFromJson(const std::filesystem::path &path_to_config) {
  try {
    std::optional<ConfigPtr> result =
        ConstructConfigFromJson(std::move(ParseJson(path_to_config)));
    return result;
...
std::optional<ConfigPtr> ConstructConfigFromYaml(YAML::Node &&doc) {
  ...
  if (!doc["threads"].IsNull()) {
    res->threads =
        std::max<unsigned int>(res->threads, doc["threads"].as<unsigned int>());
  }

  if (!doc["connection_timeout"].IsNull()) {
    res->connection_timeout = std::min<unsigned int>(
        res->connection_timeout, doc["connection_timeout"].as<unsigned int>());
  }
  if (!doc["request_timeout"].IsNull()) {
    res->request_timeout = std::min<unsigned int>(
        res->request_timeout, doc["request_timeout"].as<unsigned int>());
  }
...
std::optional<ConfigPtr>
GetConfigFromYaml(const std::filesystem::path &path_to_config) {
  try {
    std::optional<ConfigPtr> result =
        ConstructConfigFromYaml(std::move(ParseYaml(path_to_config)));
    return result;
...
std::optional<ConfigPtr>
ParseConfigFile(const std::filesystem::path &config_filepath) {

  {
    std::error_code ec;
    if (!std::filesystem::exists(config_filepath, ec)) {
      spdlog::error("Config file: {} does not exists\nError code message: {}",
                    config_filepath, ec.message());
      return std::nullopt;
    }
  }
